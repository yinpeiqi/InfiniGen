...
def run_flexgen(args):
    ...
    out_path = "results/arxiv_result_{1}_{2}".format(
        select_method, model_index, args.alpha)

    print(f"<run_infinigen>: args.model: {args.model}, dataset: ArxivSum, alpha: {args.alpha}")
    ...
    for idx in tqdm(range(200)):
        prompt = prompt_format.format(dataset['article'][idx])
